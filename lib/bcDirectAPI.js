        ...
        batch.map(async (orderId) => {
          try {
            ...
            return (Array.isArray(data) ? data : []).map(item => {
              const options = Array.isArray(item.product_options) ? item.product_options : [];
...
export async function fetchProductCatalog() {
  return cachedFetch('product_catalog_v2', async () => {
    const [categories, brands, products, variants] = await Promise.all([
      fetchAllBC('/v2/categories'),
      fetchAllBC('/v2/brands'),
      fetchAllBC('/v3/catalog/products', { include: 'custom_fields' }),
      fetchAllBC('/v3/catalog/variants'),
    ]);
    ...
    // Build SKU → variant label map from the full variant catalog.
    // This is the most reliable source — works even when product_options
    // is missing from order line items.
    const variantLabelMap = {};
    variants.forEach(v => {
      if (v.sku && Array.isArray(v.option_values) && v.option_values.length > 0) {
        variantLabelMap[v.sku] = v.option_values.map(o => o.label).filter(Boolean).join(' / ');
      }
    });
