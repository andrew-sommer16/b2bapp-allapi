...
// TTLs: slow-changing data (companies, products, reps) = 5 mins
//       orders (can change more often) = 2 mins
const cache = new Map();
...
export async function fetchAllCompanies() {
  ...
  cacheSet('companies', result, TTL.COMPANIES);
  return result;
}
...
export async function fetchAllSalesReps() {
  const cached = cacheGet('sales_reps');
  if (cached) return cached;
  ...
  cacheSet('sales_reps', result, TTL.SALES_REPS);
  return result;
}

// ─── All customer groups ───────────────────────────────────────────────────────
export async function fetchAllCustomerGroups() {
  const cached = cacheGet('customer_groups');
  if (cached) return cached;
  ...
  cacheSet('customer_groups', result, TTL.CUSTOMER_GROUPS);
  return result;
}

// ─── B2B orders for a list of company IDs ─────────────────────────────────────
export async function fetchB2BOrdersForCompanies(companyIds, { dateFrom, dateTo } = {}, skipCache = false) {
...
export async function fetchLineItemsForOrders(orderIds, batchSize = 10) {
  ...
  cacheSet(cacheKey, allItems, TTL.LINE_ITEMS);
  return allItems;
}

// ─── Product catalog with brand + category names ───────────────────────────────
export async function fetchProductCatalog() {
  const cached = cacheGet('product_catalog');
  if (cached) return cached;
  ...
  cacheSet('product_catalog', result, TTL.PRODUCT_CATALOG);
  return result;
}
